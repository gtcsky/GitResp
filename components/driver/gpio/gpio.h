...
// enable or disable gpio analog mode
typedef enum
{
	Bit_DISABLE = 0,
	Bit_ENABLE,
...
{
	GPIO_Pin_e pin;
	BitAction_e bit_action;
	GPIO_ioe ioe;	
	IO_Pull_Type_e pull_type;
	IO_Wakeup_Pol_e wakeup_pol;
	Fmux_Type_e fmux_type;
	gpioin_wakeup_t wakeup_t;
}gpio_struct_t;
...
int userGpioInterruptEnable(GPIO_Pin_e pin, IO_Wakeup_Pol_e type);

int userGpioInterruptDisable(GPIO_Pin_e pin) ;
